...
void OAuthTwitter::authorizeXAuth(const QString &username, const QString &password)
{
    ...
    QByteArray oauthHeader = generateAuthorizationHeader(url, OAuth::POST, AccessToken);

    QNetworkRequest req(url);
    req.setRawHeader(AUTH_HEADER, oauthHeader);
    //req.setRawHeader("User-Agent", TBD);
...
{
	qDebug()<<"OAuthLinkedIn::finishedAuthorization()";
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(sender());
    if (reply) {
        if (reply->error() == QNetworkReply::NoError) {
        	qDebug()<<"OAuthLinkedIn::finishedAuthorization(): no error";
            ...
            //dump error
        	qDebug()<<"OAuthLinkedIn::finishedAuthorization(): error";
...
/**
 *  request a requestToken so that we can use user credentials to authorize the usages.
 */
void OAuthTwitter::requestRequestToken()
{
    Q_ASSERT(m_netManager != 0);

    QUrl url(TWITTER_REQUEST_TOKEN_URL);

    QByteArray oauthHeader = generateAuthorizationHeader(url, OAuth::POST,RequestToken);
    ...
    if (t.isActive()) {
    	qDebug() << "pasring response for request token";
        t.stop();
        QByteArray response = reply->readAll();
        parseTokens(response, false);
...
void OAuthTwitter::requestAuthorization()
{
    ...
    authorizeUrl.addEncodedQueryItem("oauth_token", oauthRequestToken());
