...
#ifndef CONSUMER_KEY
    #define CONSUMER_KEY "UNUSED"
#endif //CONSUMER_KEY

#ifndef CONSUMER_SECRET
    #define CONSUMER_SECRET "UNUSED"
 ...
 */
void OAuth::parseTokens(const QByteArray& response, bool isAccesToken)
{
    //OAuth spec 5.3, 6.1.2, 6.3.2
    //use QUrl for parsing
    QByteArray parseQuery("");
    ...
    {
        m_oauthRequestToken = parseUrl.encodedQueryItemValue("oauth_token");
        m_oauthRequestTokenSecret  = parseUrl.encodedQueryItemValue("oauth_token_secret");
...
QByteArray OAuth::oauthRequestToken() const
{

	return m_oauthRequestToken;

}
QByteArray OAuth::oauthRequestTokenSecret() const
{

	return m_oauthRequestTokenSecret;
}
 ...
 */
QByteArray OAuth::generateSignatureHMACSHA1(const QByteArray& signatureBase, AuthorizationType authorization)
{
    //OAuth spec. 9.2 http://oauth.net/core/1.0/#anchor16
    QByteArray key = determineSigningKey( authorization);
...
QByteArray OAuth::determineSigningKey(AuthorizationType authorization)
{
	 QByteArray signingKey;
	    ...
	    {
	    case RequestToken:
	    	signingKey = m_oauthConsumerSecret+'&';
	    	break;
	    case AccessToken:
	    	signingKey = m_oauthConsumerSecret+'&'+m_oauthRequestTokenSecret;
	    	break;
	    case APICall:
	    	signingKey = m_oauthConsumerSecret+'&'+m_oauthTokenSecret;
	    	break;
	    }
	return signingKey;
}
		...
		AuthorizationType authorization)
{
    ...
    while(i.hasNext()){
            ...
            QByteArray normItem = queryItem.first + '=' + (queryItem.second.contains(',') ? queryItem.second.toPercentEncoding() : queryItem.second);
    ...
    {
	case RequestToken:
	    normParameters.append(QByteArray("oauth_callback=oob"));
		break;
	case AccessToken:
		normParameters.append(QByteArray("oauth_token=") + m_oauthRequestToken);
    ...
    //OAuth spec. 9.1.3
    //return  normUrl.toPercentEncoding() + '&' + normString;
 ...
 */
QByteArray OAuth::generateAuthorizationHeader( const QUrl& url, HttpMethod method, AuthorizationType authorization )
{
    if (authorization==APICall &&m_oauthToken.isEmpty() && m_oauthTokenSecret.isEmpty())
        qDebug() << "OAuth tokens are empty!";

    QByteArray timeStamp = generateTimeStamp();
    QByteArray nonce = generateNonce();

    QByteArray sigBase = generateSignatureBase(url, method, timeStamp, nonce,authorization);
    QByteArray signature = generateSignatureHMACSHA1(sigBase, authorization);
    //QByteArray signature = this->determineSigningKey(authorization);
	...
	case RequestToken:
		...
		break;
	case AccessToken:
		header += "oauth_token=\"" + m_oauthRequestToken + "\",";
		break;
	case APICall:
